...
chrome.storage.sync.get(['ruffle_enable', 'ignore_optout'], async function (data) {
    ...
    document.addEventListener(obfuscated_event_prefix + "_response", function (e) {
    ...
    function next_response() {
        if (next_response_promise == null) {
            next_response_promise = new Promise(function (resolve, reject) {
    ...
    async function marshal_message_into_untrusted_world(request) {
        let req_event = new CustomEvent(obfuscated_event_prefix + "_request", {"detail": JSON.stringify(request)});
